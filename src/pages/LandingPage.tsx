...
const features = [
  ...
  { icon: Shield, title: "Segurança", description: "Seus dados protegidos com criptografia e autenticação segura." },
  { icon: BarChart3, title: "Relatórios", description: "Exporte relatórios completos em PDF para análise detalhada." },
];

const faqs = [
  { q: "O Nexo é gratuito?", a: "Sim! Você pode usar o plano gratuito com funcionalidades limitadas. Para acesso completo, assine o plano PRO por apenas R$14,90/mês." },
  { q: "Meus dados estão seguros?", a: "Absolutamente. Utilizamos criptografia de ponta e autenticação segura para proteger todas as suas informações financeiras." },
  { q: "O que a IA financeira faz?", a: "Nossa IA analisa seus gastos, receitas e investimentos para oferecer dicas personalizadas de economia e organização financeira." },
  { q: "Posso cancelar a qualquer momento?", a: "Sim, você pode cancelar sua assinatura PRO a qualquer momento, sem taxas de cancelamento." },
];
  ...
  return (
    <div className="min-h-screen bg-background">
      <nav className="sticky top-0 z-50 border-b bg-card/80 backdrop-blur-sm">
        <div className="container mx-auto flex h-16 items-center justify-between px-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button asChild className="gradient-primary border-0"><Link to="/register">Criar Conta Grátis</Link></Button>
          </div>
        </div>
      </nav>
        ...
        <div className="container mx-auto px-4 text-center">
          <motion.div initial="hidden" animate="visible" variants={fadeUp} custom={0}>
            <span className="inline-flex items-center gap-2 rounded-full border bg-card px-4 py-1.5 text-sm font-medium text-muted-foreground mb-6">
              <Sparkles className="h-4 w-4 text-primary" /> Gestão financeira com inteligência artificial
            </span>
          </motion.div>
          <motion.h1 className="font-display text-4xl md:text-6xl lg:text-7xl font-extrabold tracking-tight mb-6 max-w-4xl mx-auto" initial="hidden" animate="visible" variants={fadeUp} custom={1}>
            Pare de perder dinheiro.{" "}<span className="text-gradient">Organize suas finanças</span> de vez.
          </motion.h1>
          <motion.p className="text-lg md:text-xl text-muted-foreground max-w-2xl mx-auto mb-10" initial="hidden" animate="visible" variants={fadeUp} custom={2}>
            Controle despesas, receitas e investimentos com gráficos inteligentes e dicas personalizadas da nossa IA.
          </motion.p>
        ...
        <div className="container mx-auto px-4">
          <div className="text-center mb-16">
            <h2 className="font-display text-3xl md:text-4xl font-bold mb-4">
              Tudo que você precisa para <span className="text-gradient">controlar seu dinheiro</span>
            </h2>
            <p className="text-muted-foreground text-lg max-w-2xl mx-auto">Ferramentas poderosas para organizar sua vida financeira.</p>
          </div>
      ...
      <section id="planos" className="py-20 md:py-28 bg-muted/30">
        <div className="container mx-auto px-4">
          <div className="text-center mb-16">
            <h2 className="font-display text-3xl md:text-4xl font-bold mb-4">Escolha o plano ideal para você</h2>
            <p className="text-muted-foreground text-lg">Comece grátis e evolua quando quiser.</p>
          </div>
            ...
            <Card className="relative">
              <CardContent className="p-8">
                ...
                <ul className="space-y-3 mb-8">
                  {["50 registros de despesas", "50 registros de receitas", "10 investimentos", "Gráficos básicos"].map((item) => (
                    <li key={item} className="flex items-center gap-2 text-sm"><Check className="h-4 w-4 text-primary shrink-0" />{item}</li>
                  ))}
                </ul>
                <Button variant="outline" className="w-full" asChild><Link to="/register">Começar Grátis</Link></Button>
              </CardContent>
            </Card>
            <Card className="relative border-primary shadow-lg">
              <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                <span className="gradient-primary text-primary-foreground text-xs font-semibold px-4 py-1 rounded-full">MAIS POPULAR</span>
              </div>
              <CardContent className="p-8">
                ...
                <ul className="space-y-3 mb-8">
                  {["Registros ilimitados", "Relatórios completos", "Gráficos avançados", "Exportar em PDF", "Agente financeiro IA", "Suporte prioritário"].map((item) => (
                    <li key={item} className="flex items-center gap-2 text-sm"><Check className="h-4 w-4 text-primary shrink-0" />{item}</li>
                  ))}
                </ul>
                <Button className="w-full gradient-primary border-0" asChild><Link to="/register">Assinar PRO</Link></Button>
              </CardContent>
            </Card>
          </div>
        </div>
      ...
      <section className="py-20 gradient-primary">
        <div className="container mx-auto px-4 text-center">
          <h2 className="font-display text-3xl md:text-4xl font-bold text-primary-foreground mb-4">Comece a organizar suas finanças agora</h2>
          <p className="text-primary-foreground/80 text-lg mb-8 max-w-xl mx-auto">
            Junte-se a quem já transformou sua vida financeira com o Nexo.
          </p>
